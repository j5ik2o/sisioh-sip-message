...
class ServerSpec extends Specification {
  "Server" should {
    ...
    "エンコード結果を取得できること" in {
      ...
      target.encodeByJson() must_== """{"headerName":"Server","serverVals":[{"type":"product","value":"PRODUCT1"}]}"""
      target.addProduct(Product("PRODUCT2")).encode() must_== """Server: PRODUCT2 PRODUCT1""" + Separators.NEWLINE
      target.addProduct(Product("PRODUCT2")).encodeByJson() must_== """{"headerName":"Server","serverVals":[{"type":"product","value":"PRODUCT2"},{"type":"product","value":"PRODUCT1"}]}"""
    }
  }
}
